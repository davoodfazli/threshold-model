...
double		ka=1,
		...
		sigma=1.5	,//incease factor for threshold limit
				pay[N],//pay off
				gama=2.6,
				alpha = 1/(gama - 1),
				p_change;
...
//cout << " total degree2 = " <<endl;
	     ...
	     deg[x1]++;
   	     deg[x2]++;
	     total_degree +=2;
//	     cout << " total degree2 = " << total_degree << endl;
	   }
	 }
...
for(int m=0;m<M;m++){
 ...
 // if(i==1)cout<<"i= "<<i<<endl;  
			    ...
			    site1=adj[i][0];
//  if(i==1)cout<<"i2= "<<site1<<endl;
			    for(int l=0 ;l<deg[site1];l++){
			      if (adj[site1][l]==i) node=l;
			    }
//cout<<"i=  "<<i<<"   ,nod=  "<<node<<"site=  "<<site1<<endl;
			    prob=ran2(&seed);
			    site=ran2(&seed)*N;
//			    cout<<site<<endl;
			    test=0;

			    for (int f=0;f<deg[i];f++){
			      if (adj[i][f]==site) test=1;
			    }
//cout<<"i2= "<<site1<<endl;
//			    cout<< "ok"<<endl;
			    if(prob<=0.0.2 && test==0 && deg[i]>0 && deg[site1]>0){
			      adj[i].erase(adj[i].begin()+0);
			      adj[site1].erase(adj[site1].begin()+node);
			      adj[i].push_back(site);
			      adj[site].push_back(i);
			      deg[site]++;
			      deg[site1]--;
//cout<<"i=  "<<i<<"   ,nod2=  "<<node<<endl;
...
//###################
	...
	cout << " gp = " << gp << endl;
	cout << " deg[hub] = " << adj[N-1].size() << endl;
for(int i=0;i<N;i++){
bin[deg[i]]++;
}
for(int i=0;i<20;i++){
//outfstr[7] << setprecision(8) << i << "\t" <<  bin[i]<< endl;
}
	cout << " gp1 = " << gp << endl;	
...
//		outfstr[2] << setprecision(8) << k << "\t" <<  g1 / double(gp*1.0) << endl;
	...
	if((g*1.0/N)>0.2) fact++;
		...
		int g_t_p=0;
		  ...
		  for(int i=0;i<100;i++){//time average
...
//		  }
		...
		outfstr[1] << setprecision(8) << alfa << "\t" << g*1.0 / double(gp*1.0) << endl;
//		outfstr[2] << setprecision(8) << alfa << "\t" << g1*1.0 / double(gp*1.0)<< endl;
//		outfstr[1] << setprecision(8) << alfa << "\t" << G[900][sample]*1.0 / double(gp*1.0) << endl;
		outfstr[2] << setprecision(8) << alfa << "\t" << G1[300][sample]*1.0 / double(gp*1.0)<< endl;
		outfstr[2] << setprecision(8) << alfa << "\t" << G1[301][sample]*1.0 / double(gp*1.0) << endl;
...
//		outfstr[1] << setprecision(8) << alfa << "\t" << g3_t*1.0 / double(gp*1.0*E*100) << endl;
//		outfstr[2] << setprecision(8) << alfa << "\t" << g4_t*1.0 / double(gp*1.0*E) << endl;
//		outfstr[2] << setprecision(8) << alfa << "\t" << g5_t*1.0 / double(gp*1.0*E) << endl;
